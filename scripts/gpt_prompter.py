...
class PromptGenerator:
    ...
    def __getParamDescStringForMethod(self, param):
        ...
        if valrange:
            ...
            res += f'This will take values from the following {", ".join(opts[:-1])} and {opts[-1]}. Default value will be {default}. '
    ...
    def __getMethodDescString(self, method):
        ...
        for param in params:
            val = params[param]
            res += "\t\t\t" + param + ": " + self.__getParamDescStringForMethod(val) + '\n'
        return res
    ...
    def generateBasePrompt(self):
        ...
        prompt = f'''   Consider the following robot named - {desc['name']}.
                        ...
                        Note: All parameters to the high level operations described are requeired.
...
class GPTPrompterService:
    ...
    def __callBasePrompt(self):
        try:
            ...
            print(f"An exception occured in Base Prompt Generation")
